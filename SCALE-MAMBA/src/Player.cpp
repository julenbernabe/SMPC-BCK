...
int main(int argc, const char *argv[])
{
  ...
  if (allArgs.size() != 3)
    ...
    {
      // Recall that when we run a program, we introduce the command "./Player.x 0 Programs/<name>". This is what is being read in this block.
      my_number= (unsigned int) atoi(allArgs[1]->c_str());    // Here the program reads the number of player that has run the Player.x
      progname= *allArgs[2];                                  // Here the programs reads the name of the program introduced (e.g. "Programs/mult")
    }
  ...
  /* Initialize SSL */
  SSL_CTX *ctx;
  Init_SSL_CTX(ctx, my_number, SD);                 // Inside this command we are loading the certificates of each player
  ...
  //unique_ptr<Input_Output_Tecnalia> io(new Input_Output_Tecnalia);
